...
} else if (( isset($_GET['email']) AND empty($_GET['email']) ) || ((strlen($_GET['email'])) > 100)) { // verification du champ de l'addresse mail
    echo "Veuillez spécifier une adresse mail";
}else if ((preg_match("/^[-+.\w]{1,64}@[-.\w]{1,64}\.[-.\w]{2,6}$/i", $_GET['email'])) == 0) { // verification du champ de l'adresse mail
    echo "L'adresse ".$_GET['email']." n'est pas valide";
} else if (( isset($_GET['mdp']) AND empty($_GET['mdp']) ) || ((strlen($_GET['mdp'])) > 50)) { // verification du champ du mot de passe
    echo "Veuillez spécifier un mot de passe";
} else if (( isset($_GET['cmdp']) AND empty($_GET['cmdp']) ) || ((strlen($_GET['cmdp'])) > 50)) { // verification de la copie du mot de passe
    echo "Veuillez spécifier une confirmaion de mot de passe";
} else if ($_GET['mdp'] != $_GET['cmdp']) {                                                      // verification pour voir si les deux mot de passe sont identiques
...
if (isset($_GET['pseudo']) AND !empty($_GET['pseudo'])AND                                                                       //* verification si tout les champ sont ok
    ...
    try {
        ...
        $resultat = $dbh->query("SELECT id_utilisateur, pseudo, email FROM utilisateur ORDER BY id_utilisateur");   //requete sql pour savoir si on a un pseudo  ou un email déjà pris
        $check = $resultat->fetch(PDO::FETCH_NUM);
        if ($check == true) {
            foreach ($resultat as $ligne) {               //pour chaque ligne
